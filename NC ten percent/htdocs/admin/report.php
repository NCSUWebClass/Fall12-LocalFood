...
$csv='';
if ($csv) $csv .= ",";
$csv .= '"' . str_replace('"', '\"', "CONSUMER STATISTICS") . '"';
$csv .= "\r\n";
if ($csv) $csv .= ",";
...
{	
	$sumrow=0;
	$ress = __do_sql("SELECT sum(consumer_data_weekly.amount) as total_amount,sum(consumer_data_weekly.garden_amount) as garden_amount from (consumer_data_weekly inner join members on members.kind='consumer' and consumer_data_weekly.fkey=members.pkey and members.registered_on < '".$year."-".$month."-30 00:00:00' and consumer_data_weekly.answered_on < '".$year."-".$month."-30 00:00:00' and members.zipcode in (select zipcode from zipcodes where zipcodes.county='".$rr[0]."'))");
