...
function __is_admin_user($unity_id = '')
{
	...
	return strstr('|jmzobkiw|ndsouza3|rconlon|rkimsey|mccollum|ncreamer|tlwymore|cjcrawfo|jplaiosa|klfrizze|ksjayara|gmbrigha|ejballar|krbaldwi|rdstout|',$unity_id);
}
...
function __get_consumer_weekly_spend_amount()
{
	...
	$result = __do_sql("select sum(weekly_spend_amount) as total_weekly_spend_amount from consumer_data_initial");
	if ($result && mysql_num_rows($result) == 1) {
...
{
	if (__is_consumer($pkey)) {
		$result = __do_sql("select `$field` from consumer_data_initial where fkey = '$pkey' limit 1");
		if ($result && mysql_num_rows($result) == 1)
			return mysql_result($result, 0, "$field");
	} else if (__is_business_type($pkey, 'RESTAURANT')) {
...
{
	$result = __do_sql("select `$field` from business_data where fkey = '$pkey' limit 1");
	if ($result && mysql_num_rows($result) == 1)
		return mysql_result($result, 0, "$field");
	return '';
}
 ...
 * @return array
 */
function __get_consumers_in_county($zipcode, int $weeks) { 
 ...
 * @return array
 */
function __get_consumers_in_household_size(int $household_size, int $weeks) { 
	$result = __do_sql("SELECT members.pkey, members.name, consumer_data_weekly.fkey, consumer_data_weekly.answered_on, consumer_data_weekly.amount FROM members
						INNER JOIN consumer_data_initial ON members.pkey = consumer_data_initial.fkey AND members.kind = 'consumer' AND consumer_data_initial.household = '".$household_size."' 
						INNER JOIN consumer_data_weekly ON members.pkey = consumer_data_weekly.fkey AND 
						(UNIX_TIMESTAMP(consumer_data_weekly.answered_on) > (UNIX_TIMESTAMP()-(604800*".$weeks."))) ORDER BY consumer_data_weekly.answered_on");
	return $result;
}
 ...
 * @return array
 */
function __get_consumers_history(int $weeks, $pkey) { 
    echo "apr 15 <br>";
	$result = __do_sql("SELECT members.pkey, members.name, consumer_data_weekly.fkey, consumer_data_weekly.answered_on, consumer_data_weekly.amount FROM members 
