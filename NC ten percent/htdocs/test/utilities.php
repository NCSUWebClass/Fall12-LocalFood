...
function __is_admin_user($unity_id = '')
{
	...
	return strstr('|jmzobkiw|ndsouza3|wjiang4|rconlon|rkimsey|mccollum|ncreamer|tlwymore|cjcrawfo|jplaiosa|klfrizze|ksjayara|gmbrigha|ejballar|krbaldwi|rdstout|kainger2|vcheruk|bsfilipp',$unity_id);
}

function __is_edit_user($unity_id = '')
{
	if ($unity_id == '') $unity_id = __current_user_unity_id();
	return strstr('tlwymore|rdstout|cjcrawfo|vcheruk|bsfilipp',$unity_id);
}
 ...
 */
 ...
 function __get_restaurants_in_regionByCounty($county) {
	$result= __do_sql("SELECT MAX(business_partner.url) AS url,members.bname AS business FROM members , business_partner, zipcodes zc WHERE (btype='RESTAURANT' OR sbtype='RESTAURANT') AND business_partner.fkey=members.pkey AND members.active='Y' AND members.opted_out='N' AND members.zipcode=zc.zipcode AND zc.county='$county' GROUP BY members.bname");
	return $result;
 }

function __get_restaurants_in_regionByZipAndRadiusAndCounty($color, $zipCode, $radius, $county) {
	...
	for($i=0; $i<$numarr;  $i++) {
		...
		if ($zips[$i] == $zipCode) {
			...
			$maxLat = mysql_fetch_assoc( __do_sql("select max(latitude) as maxLat from zipcodes where zipcode = '$zipCode'") );
			$maxLat = $maxLat['maxLat'];
			
			$minLat = mysql_fetch_assoc( __do_sql("select min(latitude) as minLat from zipcodes where zipcode = '$zipCode'") );
			$minLat = $minLat['minLat'];
			
			$deltaLat = $minLat + (($maxLat-$minLat)/2);
			
			$a = $deltaLat - $radius/50;
			$b = $deltaLat + $radius/50;
			
			$maxLong = mysql_fetch_assoc(__do_sql("select max(longitude) as maxLong from zipcodes where zipcode = '$zipCode' "));
			$maxLong = $maxLong['maxLong'];
			
			$minLong = mysql_fetch_assoc(__do_sql("select min(longitude) as minLong from zipcodes where zipcode = '$zipCode' "));
			$minLong = $minLong['minLong'];
			
			$deltaLong = $minLong + (($maxLong-$minLong)/2);

			$c = $deltaLong - $radius/70;	
			$d = $deltaLong + $radius/70;
			
			$result= __do_sql("SELECT MAX(business_partner.url) AS url,members.bname AS business FROM members,business_partner, zipcodes WHERE (btype='RESTAURANT' OR sbtype='RESTAURANT') AND business_partner.fkey=members.pkey AND members.active='Y' AND members.opted_out='N' AND members.zipcode=zipcodes.zipcode AND zipcodes.latitude >= '$a' AND zipcodes.latitude <= '$b' AND zipcodes.longitude >= '$c' AND zipcodes.longitude <= '$d' AND zipcodes.county='$county' GROUP BY members.bname");
		}
	}
...
function __get_restaurants_in_regionByZipAndRadius($color, $zipCode, $radius) {
	...
	for($i=0; $i<$numarr;  $i++) {
		...
		if ($zips[$i] == $zipCode) {
			...
			$result= __do_sql("SELECT MAX(business_partner.url) AS url,members.bname AS business FROM members,business_partner, zipcodes WHERE (btype='RESTAURANT' OR sbtype='RESTAURANT') AND business_partner.fkey=members.pkey AND members.active='Y' AND members.opted_out='N' AND members.zipcode=zipcodes.zipcode AND zipcodes.latitude >= '$a' AND zipcodes.latitude <= '$b' AND zipcodes.longitude >= '$c' AND zipcodes.longitude <= '$d' GROUP BY members.bname");
		}
	}
...
function __get_restaurants_in_regionByZip($color, $zipCode) {
	...
	for($i=0; $i<$numarr;  $i++) {
		...
		if ($zips[$i] == $zipCode) {
			$result= __do_sql("SELECT MAX(business_partner.url) AS url,members.bname AS business FROM members,business_partner WHERE (btype='RESTAURANT' OR sbtype='RESTAURANT') AND business_partner.fkey=members.pkey AND members.active='Y' AND members.opted_out='N' AND zipcode='$zipCode' GROUP BY members.bname");
		}
	}
...
return $result;
 ...
 function __get_frmmkt_in_regionByCounty($county) {
	$result= __do_sql("SELECT MAX(business_partner.url) AS url,members.bname AS business FROM members,business_partner, zipcodes zc WHERE (btype = 'FRMMKT' OR sbtype='FRMMKT') AND business_partner.fkey=members.pkey AND members.active='Y' AND members.opted_out='N' AND members.zipcode=zc.zipcode AND zc.county='$county' GROUP BY members.bname");
	return $result;
 }

 function __get_frmmkt_in_regionByZipAndRadiusAndCounty($color, $zipCode, $radius, $county) {
	...
	for($i=0; $i<$numarr;  $i++) {
		...
		if ($zips[$i] == $zipCode) {
			...
			$result= __do_sql("SELECT MAX(business_partner.url) AS url,members.bname AS business FROM members,business_partner, zipcodes WHERE (btype = 'FRMMKT' OR sbtype='FRMMKT') AND business_partner.fkey=members.pkey AND members.active='Y' AND members.opted_out='N' AND members.zipcode=zipcodes.zipcode AND zipcodes.latitude >= '$a' AND zipcodes.latitude <= '$b' AND zipcodes.longitude >= '$c' AND zipcodes.longitude <= '$d' AND zipcodes.county='$county' GROUP BY members.bname");
		}
	}
...
function __get_frmmkt_in_regionByZipAndRadius($color, $zipCode, $radius) {
...
function __get_frmmkt_in_regionByZip($color, $zipCode) {
	...
	for($i=0; $i<$numarr;  $i++) {
		...
		if ($zips[$i] == $zipCode) {
			$result= __do_sql("SELECT MAX(business_partner.url) AS url,members.bname AS business FROM members,business_partner WHERE (btype = 'FRMMKT' OR sbtype='FRMMKT') AND business_partner.fkey=members.pkey AND members.active='Y' AND members.opted_out='N' AND zipcode='$zipCode' GROUP BY members.bname");
		}
	}
...
return $result;
 ...
 function __get_groc_in_regionByCounty($county) {
	$result= __do_sql("SELECT MAX(business_partner.url) AS url,members.bname AS business FROM members,business_partner, zipcodes zc WHERE (btype = 'RETAIL-GRO' OR btype = 'FOODSERV' OR sbtype='RETAIL-GRO' OR sbtype='FOODSERV') AND business_partner.fkey=members.pkey AND members.active='Y' AND members.opted_out='N' AND members.zipcode=zc.zipcode AND zc.county='$county' GROUP BY members.bname");
	return $result;
 }

function __get_groc_in_regionByZipAndRadiusAndCounty($color, $zipCode, $radius, $county) {
	...
	for($i=0; $i<$numarr;  $i++) {
		...
		if ($zips[$i] == $zipCode) {
			...
			$result= __do_sql("SELECT MAX(business_partner.url) AS url,members.bname AS business FROM members,business_partner, zipcodes WHERE (btype = 'RETAIL-GRO' OR btype = 'FOODSERV' OR sbtype='RETAIL-GRO' OR sbtype='FOODSERV') AND business_partner.fkey=members.pkey AND members.active='Y' AND members.opted_out='N' AND members.zipcode=zipcodes.zipcode AND zipcodes.latitude >= '$a' AND zipcodes.latitude <= '$b' AND zipcodes.longitude >= '$c' AND zipcodes.longitude <= '$d' AND zipcodes.county='$county' GROUP BY members.bname");
		}
	}
...
function __get_groc_in_regionByZipAndRadius($color, $zipCode, $radius) {
...
function __get_groc_in_regionByZip($color, $zipCode) {
...
return $result;
 ...
 function __get_farms_in_regionByCounty($county) {
	$result= __do_sql("SELECT MAX(business_partner.url) AS url, members.bname AS business FROM members,business_partner, zipcodes zc WHERE (btype='FARM' OR sbtype='FARM') AND members.pkey=business_partner.fkey AND members.active='Y' AND members.opted_out='N' AND  members.zipcode=zc.zipcode AND zc.county='$county' GROUP BY members.bname");
	return $result;
 }
 
function __get_farms_in_regionByZipAndRadiusAndCounty($color, $zipCode, $radius, $county) {
	...
	for($i=0; $i<$numarr;  $i++) {
		...
		if ($zips[$i] == $zipCode) {
			...
			$result= __do_sql("SELECT MAX(business_partner.url) AS url,members.bname AS business FROM members,business_partner, zipcodes WHERE (btype='FARM' OR sbtype='FARM') AND members.pkey=business_partner.fkey AND members.active='Y' AND members.opted_out='N' AND  members.zipcode=zipcodes.zipcode AND zipcodes.latitude >= '$a' AND zipcodes.latitude <= '$b' AND zipcodes.longitude >= '$c' AND zipcodes.longitude <= '$d' AND zipcodes.county='$county'  GROUP BY members.bname");
		}
	}
...
function __get_farms_in_regionByZipAndRadius($color, $zipCode, $radius) {
	...
	for($i=0; $i<$numarr;  $i++) {
		...
		if ($zips[$i] == $zipCode) {
			...
			$result= __do_sql("SELECT MAX(business_partner.url) AS url,members.bname AS business FROM members,business_partner, zipcodes WHERE (btype='FARM' OR sbtype='FARM') AND members.pkey=business_partner.fkey AND members.active='Y' AND members.opted_out='N' AND  members.zipcode=zipcodes.zipcode AND zipcodes.latitude >= '$a' AND zipcodes.latitude <= '$b' AND zipcodes.longitude >= '$c' AND zipcodes.longitude <= '$d' GROUP BY members.bname");
		}
	}
...
function __get_farms_in_regionByZip($color, $zipCode) {
	...
	for($i=0; $i<$numarr;  $i++) {
		...
		if ($zips[$i] == $zipCode) {
			$result= __do_sql("SELECT MAX(business_partner.url) AS url, members.bname AS business FROM members,business_partner WHERE (btype='FARM' OR sbtype='FARM') AND members.pkey=business_partner.fkey AND members.active='Y' AND members.opted_out='N' AND zipcode='$zipCode' GROUP BY members.bname");
		}
	}
...
return $result;
 ...
 function __get_community_in_regionByCounty($county) {
	$result= __do_sql("SELECT MAX(business_partner.url) AS url,members.bname AS business FROM members,business_partner, zipcodes zc WHERE (btype='COMMUNITY-' OR btype='NONPROFIT' OR btype='CORPORATIO' OR btype='OTHER' OR btype='GOVT' OR btype='SCHOOL-UNI' OR sbtype='COMMUNITY-' OR sbtype='NONPROFIT' OR sbtype='CORPORATIO' OR sbtype='OTHER' OR sbtype='GOVT' OR sbtype='SCHOOL-UNI') AND business_partner.fkey=members.pkey AND members.active='Y' AND members.opted_out='N' AND members.zipcode=zc.zipcode AND zc.county='$county' GROUP BY members.bname");
	return $result;
 }

function __get_community_in_regionByZipAndRadiusAndCounty($color, $zipCode, $radius, $county) {
	...
	for($i=0; $i<$numarr;  $i++) {
		...
		if ($zips[$i] == $zipCode) {
			...
			$result= __do_sql("SELECT MAX(business_partner.url) AS url,members.bname AS business FROM members,business_partner, zipcodes WHERE (btype='COMMUNITY-' OR btype='NONPROFIT' OR btype='CORPORATIO' OR btype='OTHER' OR btype='GOVT' OR btype='SCHOOL-UNI' OR sbtype='COMMUNITY-' OR sbtype='NONPROFIT' OR sbtype='CORPORATIO' OR sbtype='OTHER' OR sbtype='GOVT' OR sbtype='SCHOOL-UNI') AND business_partner.fkey=members.pkey AND members.active='Y' AND members.opted_out='N' AND members.zipcode=zipcodes.zipcode AND zipcodes.latitude >= '$a' AND zipcodes.latitude <= '$b' AND zipcodes.longitude >= '$c' AND zipcodes.longitude <= '$d' AND zipcodes.county='$county'  GROUP BY members.bname");
		}
	}
...
function __get_community_in_regionByZipAndRadius($color, $zipCode, $radius) {
...
function __get_community_in_regionByZip($color, $zipCode) {
...
function __get_statewide_in_regionByCounty($county)
{
	$result = __do_sql("SELECT MAX(business_partner.url) AS url,members.bname AS business FROM members,business_partner, zipcod
	$zipgreen=array(28604,28616,28622,28646,28652,28653,28657,28662,28664,28701,28704,28709,28711,28715,28728,28730,28748,28757,28770,28776,28778,28787,28801,28802,28803,28804,28805,28806,28810,28813,28814,28815,28816,28781,28901,28903,28905,28906,28902,28904,28909,28733,28771,28716,28721,28738,28745,28751,28785,28786,28710,28724,28726,28727,28729,28731,28732,28735,28739,28742,28758,28759,28760,28784,28790,28791,28792,28793,28707,28717,28723,28725,28736,28779,28783,28788,28789,28734,28741,28744,28763,28775,28734,28741,28744,28763,28775,28743,28753,28754,28737,28749,28752,28761,28762,28705,28765,28777,28722,28750,28756,28773,28782,28702,28713,28719,28708,28712,28718,28747,28766,28768,28772,28774,28605,28607,28608,28618,28679,28691,28692,28698,28714,28740,28755);
	$zipred=array(27201,27202,27215,27216,27217,27244,27253,27258,27298,27302,27340,27349,27359,27212,27291,27305,27311,27314,27315,27379,27207,27208,27213,27228,27252,27256,27312,27344,27559,27239,28292,27293,28294,27295,27299,27351,27360,27361,27373,27374,27503,27572,27701,27702,27703,27704,27705,27706,27707,27708,27709,27710,27711,27712,27713,27715,27717,27722,27009,27010,27012,27023,27040,27045,27050,27051,27094,27098,27099,27101,27102,27103,27104,27105,27106,27107,27108,27109,27110,27111,27113,27114,27115,27116,27117,27120,27127,27130,27150,27152,27155,27157,27198,27199,27284,27285,27508,27525,27549,27596,27507,27509,27522,27564,27565,27581,27582,27214,27233,27235,27249,27260,27261,27262,27263,27264,27265,27282,27283,27301,27310,27313,27342,27357,27358,27377,27401,27402,27403,27404,27405,27406,27407,27408,27409,27410,27411,27412,27413,27415,27416,27417,27419,27420,27425,27427,27429,27435,27438,27455,27495,27495,27497,27498,27499,27231,27243,27278,27510,27514,27515,27516,27517,27599,27343,27541,27573,27574,27583,27203,27204,27205,27230,27248,27316,27317,27341,27350,27355,27370,27025,27027,27048,27288,27289,27320,27326,27375,27016,27019,27021,27022,27042,27043,27046,27052,27536,27537,27544,27553,27556,27584,27502,27511,27512,27513,27518,27519,27523,27526,27529,27502,27511,27512,27513,27518,27519,27523,27526,27529,27539,27540,27545,27560,27562,27571,27587,27588,27591,27592,27597,27601,27602,27603,27604,27605,27606,27607,27608,27609,27610,27611,27612,27613,27614,27615,27616,27617,27619,27620,27621,27622,27623,27624,27625,27626,27627,27628,27629,27634,27635,27636,27640,27650,27656,27658,27661,27668,27675,27676,27690,27695,27697,27698,27699,27551,27563,27570,27586,27589,27594);
	...
	for($i=0; $i<$numarr;  $i++) {
		...
		if ($zips[$i] == $zipCode) {
			...
			$result= __do_sql("SELECT MAX(business_partner.url) AS url,members.bname AS business FROM members,business_partner, zipcodes WHERE members.statewide='Y' AND business_partner.fkey=members.pkey AND members.active='Y' AND members.zipcode=zipcodes.zipcode AND zipcodes.latitude >= '$a' AND zipcodes.latitude <= '$b' AND zipcodes.longitude >= '$c' AND zipcodes.longitude <= '$d' GROUP BY members.bname");
		}
	}
	$num=mysql_num_rows($result);
	return $result;
...
function __get_statewide_in_regionByZip($zipCode)
{
	$result = __do_sql("SELECT MAX(business_partner.url) AS url,members.bname AS business FROM members,business_partner WHERE members.statewide='Y' AND business_partner.fkey=members.pkey AND members.active='Y' AND zipcode = '$zipCode' AND members.opted_out='N' "." GROUP BY members.bname");
	$num=mysql_num_rows($result);
	return $result;
}
